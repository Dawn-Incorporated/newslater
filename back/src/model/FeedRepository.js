...
class FeedRepository extends AbstractRepository {
    ...
    create(req, res) {
        database.execute("INSERT INTO " + this.getTable() + " (url, name, description, website) VALUES (?, ?, ?, ?)", [req.body.url, req.body.name, req.body.description, req.body.website], (results) => {
            res.json(results);
        });
    }
