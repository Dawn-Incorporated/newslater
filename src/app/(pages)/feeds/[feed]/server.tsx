...
export async function FeedHeader({feed}: {feed: string}) {
	...
	return (
		<div className="flex max-md:flex-col flex-row max-md:items-center justify-between my-6">
			<h1 className={ cn("flex text-4xl max-md:text-3xl max-md:mb-6 font-bold", libre_baskerville.className) }>{ data.name ?? data.url ?? "" }</h1>
...
export async function FeedContent({url}: {url: string}) {
	...
	return (
		<div className="grid md:grid-cols-2 gap-6 max-h-screen text-pretty break-all overflow-y-auto">
