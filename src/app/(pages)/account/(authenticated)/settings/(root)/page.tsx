import UserSettings, {
  schemaUserSettings,
} from "@/app/(pages)/account/(authenticated)/settings/(root)/client";
...
async function UserSettingsWrapper({ user_id }: { user_id: string }) {
  const user = (
    await db.select().from(auth_users).where(eq(auth_users.id, user_id))
  )[0];
...
async function editAccount(values: z.infer<typeof schemaUserSettings>) {
  ...
  if (!user_id) {
    log(
      "ERROR",
      "User settings",
      `Attempted to edit account without user_id set.`,
    );
    return;
  }
  ...
  await db
    ...
    .catch((error) => {
      log(
        "ERROR",
        "DB",
        `Error during account update. Context: ${JSON.stringify(error)}`,
      );
    })
    .finally(() => {
