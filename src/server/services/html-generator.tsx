      ...
      {feeds.map((feed, index) =>
        feed && feed.length > 0 ? (
          <>
            <h1
              className={cn(
                "mb-5 flex justify-center text-3xl font-bold",
      ...
      {publications.map((publication, index) => (
        <div key={index} className={"mb-6"}>
          <Card>
            ...
            <CardContent>
              <div key={index}>{makeSubtitles(publication)}</div>
            </CardContent>
            <CardFooter>
              <div
                className={"feed-content leading-6 text-neutral-600"}
...
function makeSubtitles(publication: Publication): (string | JSX.Element)[] {
  ...
  if (publication.pubDate) {
    const formattedDate = new Date(publication.pubDate).toLocaleString(
      "en-US",
      {
        weekday: "long",
        year: "numeric",
